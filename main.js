...
const inventorsFullName = inventors.map(function (inventors) {
...
const ageOfInventors = inventors.map(function (inventors) {
  return inventors.passed - inventors.year;
});
const ageAllTogether = ageOfInventors.reduce((a, b) => a + b, 0);
console.log(ageAllTogether);
...
// 6. create a list of Boulevards in Paris that contain
// 'de' anywhere in the name
...
const sortAlphabetically = people.sort((last, next) => {
...
const sumEach = data.reduce(function(object, item) {
    if (!object[item]) {
        object[item] = 0;
    }
    object[item]++;
